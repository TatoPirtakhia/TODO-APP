...
function Main(props) {
  ...
  const changeToactive = (event) => {
    if (event.target.classList.contains("check") || event.target.classList.contains("check_dark")) {
      ...
      <div className={props.dark?"dark_container":"container"}>
        <main className={props.dark?"dark_main":"main"}>
            ...
            return (
              <>
                <div className={props.dark?"dark_box":"box"}>
                  ...
                  <p className={element.active ? "ckecked_text" : props.dark ?"dark_text": "text"}>
                    {element.text}
                  </p>
                  <svg
                    xmlns="http://www.w3.org/2000/svg"
          ...
          <div className="components">
            <p className={props.dark?"items_left_dark":"items_left"}>{`${props.tasks.length} items left`}</p>
            <button className={props.dark?"button_clear_dark":"button_clear"} onClick={clearCompleted}>
              Clear Completed
            </button>
          </div>
        </main>
